      ...
      <div class="content-wrapper">
        <section class="content-header">
          <div class="container-fluid">
            <div class="row mb-2">
              <div class="col-sm-6">
                <h1 class="display-4">Edit Rooms</h1>
              </div>
              <div class="col-sm-6">
            ...
            <div class="row">
              <div class="col-md-8 offset-md-2">
                <div class="card shadow-sm">
                  ...
                  <div class="card-body">
                  <form method="POST" action="{{ route('hotels.rooms.update', [$hotel, $room]) }}" enctype="multipart/form-data">
                  @csrf
                  @method('PUT')
                  ...
                  <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="name" name="name" value="{{ $room->name }}" placeholder="Enter room name" required>
                  </div>
                  ...
                  <div class="form-group">
                      ...
                      <select class="form-control" id="type" name="type" required>
                          ...
                          @foreach($room_types as $id => $type)
                              <option value="{{ $id }}" {{ $room->type == $id ? 'selected' : '' }}>{{ $type->name }}</option>
                          @endforeach
                      </select>
                  </div>
                  ...
                  <div class="form-group">
                      ...
                      <input type="number" class="form-control" id="price" name="price" value="{{ $room->price }}" step="0.01" placeholder="Enter price" required>
                  </div>
                  ...
                  <div class="form-group row">
                      <div class="col-md-6">
                          ...
                          <select class="form-control" id="adults" name="adults" required>
                              <option value="1" {{ $room->adults == 1 ? 'selected' : '' }}>1 Adult</option>
                              <option value="2" {{ $room->adults == 2 ? 'selected' : '' }}>2 Adults</option>
                              <option value="3" {{ $room->adults == 3 ? 'selected' : '' }}>3 Adults</option>
                          </select>
                      </div>
                      <div class="col-md-6">
                          <label for="children" class="font-weight-bold">Number of Children:</label>
                          <select class="form-control" id="children" name="children">
                              <option value="0" {{ $room->children == 0 ? 'selected' : '' }}>No Children</option>
                              <option value="1" {{ $room->children == 1 ? 'selected' : '' }}>1 Child</option>
                              <option value="2" {{ $room->children == 2 ? 'selected' : '' }}>2 Children</option>
                          </select>
                      </div>
                  </div>
                              ...
                              <div class="col-md-3 col-sm-6">
                                  <div class="form-check">
                                      <input class="form-check-input" type="checkbox" name="benefits[]" value="{{ $benefit }}" id="benefit_{{ $loop->index }}" 
                                          {{ in_array($benefit, json_decode($room->benefits, true)) ? 'checked' : '' }}>
                  ...
                  <div class="form-group">
                      ...
                      @foreach(json_decode($room->facilities, true) as $facility)
                          <input type="text" class="form-control mb-2" name="facilities[]" value="{{ $facility }}" placeholder="Enter facility">
                      @endforeach
